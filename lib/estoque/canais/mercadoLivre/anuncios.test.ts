...
describe("pausarAnuncio", () => {
  afterEach(() => vi.unstubAllGlobals());

  it("pausa o anúncio (status: paused)", async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal("fetch", fetchMock);

    await pausarAnuncio("MLB999");
  ...
  it("lança erro quando a API responde com falha", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: false, status: 400 }));
    await expect(pausarAnuncio("MLB999")).rejects.toThrow("HTTP 400");
  });
});

describe("reativarAnuncio", () => {
  afterEach(() => vi.unstubAllGlobals());

  it("reativa o anúncio (status: active)", async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal("fetch", fetchMock);

    await reativarAnuncio("MLB999");
  ...
  it("lança erro quando a API responde com falha", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: false, status: 400 }));
    await expect(reativarAnuncio("MLB999")).rejects.toThrow("HTTP 400");
  });
});
