...
/**
 * Pausa o anúncio no Mercado Livre (`status: "paused"`) — diferente de
 * `despublicarAnuncio` (`status: "closed"`, praticamente definitivo): pausar
 * some da vitrine mas mantém o anúncio ativo, podendo ser reativado a
 * qualquer momento via `reativarAnuncio`, sem perder o `item_id`/histórico.
 */
export async function pausarAnuncio(itemId: string): Promise<void> {
  await alterarStatusAnuncio(itemId, "paused", "Falha ao pausar anúncio no Mercado Livre");
}

/** Reativa um anúncio pausado (`status: "active"`) — inverso de `pausarAnuncio`. */
export async function reativarAnuncio(itemId: string): Promise<void> {
  await alterarStatusAnuncio(itemId, "active", "Falha ao reativar anúncio no Mercado Livre");
}
...
async function alterarStatusAnuncio(
  ...
  if (!resposta.ok) {
    throw await erroMercadoLivre(resposta, mensagemErro);
  }
}
