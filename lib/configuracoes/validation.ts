...
/** Valida o payload do padrão global de taxas (EDI-106): taxas 0 ≤ x < 100, margens ≥ 0, taxa fixa ≥ 0, todos obrigatórios. */
export function validarTaxasCanais(payload: unknown): ErrosTaxasCanais {
  ...
  // Margens são sobre o custo, não sobre o preço — sem teto de 100% (200%, 300% etc. são válidos).
  const margemMinima = dados.margemMinimaPercentual;
  if (!numeroFinito(margemMinima) || margemMinima < 0) {
    erros.margemMinimaPercentual = "Informe um percentual válido, maior ou igual a 0.";
  }
