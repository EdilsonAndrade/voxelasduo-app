...
- [ ] T030 Decisão de negócio pendente: para o anúncio `MLB5203603089` (e qualquer outro já ativo com frete impreciso), escolher entre (a) editar peso/dimensões manualmente pelo painel web do Mercado Livre, se essa opção permitir o que a API não permite, ou (b) despublicar e publicar de novo pelo admin (novo `item_id`, perde histórico de vendas/avaliações do anúncio antigo) — **decisão do usuário/operador**, não decidida automaticamente
