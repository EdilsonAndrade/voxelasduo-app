...
**Correção aplicada**: `atributosEmbalagem()` passou a enviar valores numéricos puros; nova função `dimensoesEnvioParaFrete(embalagem): string` monta o `"AxBxC,peso"`; `criarAnuncio()` e `atualizarAtributosAnuncio()` passaram a incluir `shipping: { dimensions }` no corpo da requisição (via novo helper `corpoEnvio()`), além dos atributos. A ordem dos eixos (comprimento x largura x altura) segue a convenção mais comum entre integradores — a FAQ não especifica a ordem exata; **ainda precisa ser validada empiricamente** publicando/corrigindo um item real e conferindo se o frete cotado muda como esperado (mesmo padrão de descoberta já usado neste projeto).
