...
1. Resolve `category_id`: override manual por categoria do site se configurado, senão o previsor automático do Mercado Livre (`preverCategoriaMercadoLivre`) a partir do nome do produto **+ um qualificador em português da categoria do site** (`montarConsultaPrevisor`, research.md #5 — sem o qualificador, uma palavra genérica no nome pode cair num domínio errado, ex: "Chaveiro" sozinho caiu em "Antiguidades e Coleções"). Sem correspondência → registra falha em `publicacoesCanalFalhas` (`operacao: "criar"`) e retorna erro.
