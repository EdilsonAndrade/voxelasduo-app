...
- **T035 (correção pós-implementação, encontrada testando em produção 2026-09-05)**: mesmo com `family_name` e categoria correta, `POST /items` falhou com `attributes are required` ao tentar montar o título (domínio "decorations" — confirma que a categoria via previsor está certa). Criado `lib/estoque/canais/mercadoLivre/atributos.ts`: `buscarAtributosObrigatorios(categoryId)` consulta `GET /categories/{categoryId}/attributes` e filtra `tags.required`; `valorPadraoAtributo` preenche cada um por melhor esforço (lista fechada → opção genérica se existir, senão a primeira; texto livre → nome do produto). `criarAnuncio` passou a enviar `attributes` no `POST /items`. Novo teste `atributos.test.ts`; `anuncios.test.ts`, `research.md` #4 e `contracts/mercado-livre-api.md` atualizados. **Risco aceito**: heurístico — uma categoria com atributo obrigatório sem valor sensato de adivinhar ainda falha, mas de forma registrada e consultável (`GET /api/anuncios/pendencias`), nunca silenciosamente.
- **T036 (correção pós-implementação, encontrada testando em produção 2026-09-05)**: publicação funcionou de ponta a ponta, mas o previsor colocou "Chaveiro VoXElas Duo" em "Antiguidades e Coleções > Chaveiros" — a palavra "Chaveiro" sozinha tem domínio de colecionismo no Mercado Livre. Adicionado `montarConsultaPrevisor(categoria, nomeProduto)` em `categorias.ts`: acrescenta um qualificador em português derivado da categoria do site (`decoracao` → "decoração", `organizacao` → "organizador", `personalizados` → "personalizado") ao nome do produto antes de consultar o previsor. `criarAnuncio` atualizado; novo teste `categorias.test.ts`; `anuncios.test.ts` e `research.md`/`contracts/mercado-livre-api.md` (#5) atualizados.
