...
- **Pendência antes de publicar em produção**: `lib/estoque/canais/mercadoLivre/categorias.ts` (T003) foi atualizado com `MLB1574` ("Casa, Móveis e Decoração", categoria de nível 1) para `decoracao`/`organizacao`/`personalizados`, a partir da lista de categorias que o usuário passou — como é categoria raiz, o Mercado Livre pode exigir uma subcategoria mais específica para `POST /items`; se a publicação falhar por categoria, o motivo aparece em `GET /api/anuncios/pendencias` e o ajuste é só trocar o valor no arquivo.
...
- **T032 (correção pós-implementação, encontrada testando em produção 2026-09-05)**: erros do Mercado Livre só expunham o HTTP status ("HTTP 400"), escondendo o motivo real — criado `lib/estoque/canais/mercadoLivre/erros.ts` (`erroMercadoLivre`) que inclui o corpo da resposta na mensagem, aplicado em `anuncios.ts`, `client.ts` e `pedidos.ts`; novo teste `erros.test.ts`. Isso revelou o próximo erro real: `POST /items` exigia o campo `family_name` (modelo de variações do Mercado Livre, "User Products" — nome genérico da família do produto, exigido mesmo sem variação) mesmo com todos os demais campos corretos. Corrigido enviando `family_name: produto.nome` em `criarAnuncio` (research.md #4, `anuncios.test.ts` atualizado).
