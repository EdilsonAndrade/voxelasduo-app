...
**Correção pós-implementação (2026-09-05)**: testado em produção com uma categoria de nível 1 fixa (`MLB1574`), `POST /items` respondeu primeiro pedindo o campo `family_name` (`body.required_fields`) e, depois de adicioná-lo, passou a rejeitar `title` como inválido (`body.invalid_fields`) — validações incoerentes entre si, sintoma de categoria raiz/inválida para publicação direta, não de campos realmente faltando. Resolvido na raiz trocando a origem da categoria para o previsor automático (#5) em vez de tentar adicionar mais campos; com uma categoria-folha válida, o payload simples (sem `family_name`) voltou a ser o esperado. `family_name` não é enviado nesta implementação — se uma categoria específica realmente exigir no futuro (documentado como parte do fluxo de variações "User Products"), adicionar de volta só para esse caso.
